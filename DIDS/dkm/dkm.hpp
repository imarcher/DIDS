...
// only included in case there's a C++11 compiler out2 there that doesn't support `#pragma once`
#ifndef DKM_KMEANS_H
#define DKM_KMEANS_H
...
T distance_squared(const std::array<T, N>& point_a, const std::array<T, N>& point_b) {
        ...
        for (int i=0;i<len;i+=8) {
            a = _mm256_loadu_ps(&point_a[i]);
            b = _mm256_loadu_ps(&point_b[i]);
        ...
        for(int i=len;i<N;i++) {
            dis += (point_a[i] - point_b[i]) * (point_a[i] - point_b[i]);
        }
        return dis;
#else
	T d_squared = T();
...
*/
template <typename Factory, typename T, size_t N>
std::vector<uint32_t> calculate_clusters(
	const std::vector<std::array<T, N>>& data, const std::vector<std::array<T, N>>& means) {
    dids::Graph* graph = Factory::createTSVec(means.size());
    for(int i=0;i<means.size();i++) {
        graph->insert((void*)&means[i], i);
    }
	std::vector<uint32_t> clusters;
	for (auto& point : data) {
		clusters.push_back(graph->search((void*)&point, 1)[0].second);
	}
    delete graph;
	return clusters;
}
...
//	std::vector<std::array<T, N>> means = details::random_plusplus(data, parameters.get_k(), seed);
	...
	uint64_t count = 0;
	do {
		clusters = details::calculate_clusters<Factory, T, N>(data, means);
		old_old_means = old_means;
		old_means = means;
		means = details::calculate_means(data, clusters, old_means, parameters.get_k());
		++count;
        fprintf(stderr,"\r\x1b[m>> kmeans iter: \x1b[36m%2.2lf%%\x1b[0m",(float) ((float)count/(float)parameters.get_max_iteration()) * 100);
    ...
    fprintf(stderr,"\r\x1b[m>> kmeans iter: \x1b[36m%2.2lf%%\x1b[0m",(float) 100.);
    fprintf(stderr, "\n");
...
std::tuple<std::vector<std::array<T, N>>, std::vector<uint32_t>> kmeans_lloyd(
	...
	return kmeans_lloyd<Factory, T, N>(data, parameters);
}
